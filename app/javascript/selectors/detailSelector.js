...
export const selectAssignedPermissions = state => {
  const assignedPermissions = safeGet(
    state,
    'fetchDetails.details.records.user.permissions'
  );
...
const status = state =>
  safeGet(state, 'fetchDetails.details.records.user.status');
