...
export function* validateNewUser(action) {
  try {
    ...
    const verifyUserDetails = yield call(UserService.validateUser, verify.email, verify.racfid)
...
export function* validateNewUserSaga() {
  yield takeLatest(actionTypes.VALIDATE_NEW_USER_API_CALL_REQUEST, validateNewUser)
}
