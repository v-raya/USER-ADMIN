    ...
    describe '#validate_token' do
      context 'with a valid token' do
        it 'returns true' do
          ...
          allow(Faraday).to receive(:get)
            .with('https://perry.test/authn/validate?token=valid_token')
            .and_return(response)
      ...
      context 'with an invalid token' do
        it 'returns false' do
          ...
          allow(Faraday).to receive(:get)
            .with('https://perry.test/authn/validate?token=invalid_token')
            .and_return(response)
    ...
    describe '#fetch_new_token' do
      context 'with a valid access code' do
        it 'returns a valid token' do
          ...
          allow(Faraday).to receive(:get)
            .with('https://perry.test/authn/token?accessCode=someCode')
            .and_return(response)
      ...
      context 'with a invalid access code' do
        it 'returns nil' do
          allow(response).to receive(:status).and_return(401)
          allow(Faraday).to receive(:get)
            .with('https://perry.test/authn/token?accessCode=badCode')
            .and_return(response)
